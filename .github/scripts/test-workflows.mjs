...
1. Use an operator-bootstrapped Mac that is already connected to the relay. Normal testers do not enter a relay URL, PHONE_TOKEN, or MAC_TOKEN. Never put PHONE_TOKEN or MAC_TOKEN in a URL, log, screenshot, or Git.
2. In Mac Settings, choose Pair Phone. If a phone is enrolled, choose Replace Phone… and confirm.
3. Scan the nearby QR code, or choose Share Secure Setup Link… to send the HTTPS link to a phone anywhere on the internet. The link is single-use and expires after five minutes. Without the native app, use Other Ways to Connect > Copy Browser Invitation Link for the /pair/web browser fallback.
4. Confirm that both devices show the same six-digit code, then have the Mac owner approve it. The internet link never bypasses code comparison and approval; do not approve a mismatch.
...
- Taken-over state: Verify the terminal taken-over state and that the client does not auto-reconnect. Recovery requires the user to explicitly choose Save & Reconnect or reconnect.
