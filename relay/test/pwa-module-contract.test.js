...
test('every production reconnect entry delegates through credential lifecycle ownership', () => {
  ...
  assert.match(source, /window\.addEventListener\('online',[\s\S]*pairingRecovery\.online\(\)/);
  assert.match(source, /visible: \(\) => credentialLifecycle\.visible\(\)/);
  assert.match(source, /online: \(\) => credentialLifecycle\.online\(\)/);
...
test('pairing activation and Forget are credential lifecycle intents in production composition', () => {
  ...
  assert.match(source,
    /forgetPairing:\s*\(\)\s*=>\s*\{\s*pairingController\.retire\(\);\s*return credentialLifecycle\.clear\(\)/);
...
test('Forget synchronously retires claimant ownership before durable clearing', () => {
  const source = readFileSync(new URL('../public/app.js', import.meta.url), 'utf8');
  assert.match(source, /forgetPairing:\s*\(\)\s*=>\s*\{\s*pairingController\.retire\(\);\s*return credentialLifecycle\.clear\(\)/);
});
