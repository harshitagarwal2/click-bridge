...
- There is no root or relay package manifest and no package lockfile.
...
Create .gitignore and .dockerignore from Section 7, then run:
...
~~~bash
git commit -m "docs: establish click bridge v5 scope"
~~~
...
- Create: relay/src/constants.js
- Create: relay/public/wire-protocol.js
- Create: relay/test/protocol.test.js
- Create: relay/scripts/check.mjs
...
- relay exposed only to the private Compose network;
- Caddy ports 80:80 and 443:443;
- read-only bind mount ./Caddyfile:/etc/caddy/Caddyfile:ro;
- Caddy data and config named volumes;
...
- [ ] **Step 2: Validate locally before touching OCI**
...
- [ ] **Step 3: Prepare deployment inputs**
...
Use the first as PHONE_TOKEN and second as MAC_TOKEN. Store them only in /opt/click-bridge/shared/secrets.env on the VM with mode 0600 and their intended clients. Generate DIRECT_TOKEN only if Task 10 begins.

Point CLICK_BRIDGE_DOMAIN to the stable OCI public IPv4. An owned domain is the default; a dedicated DuckDNS hostname is the free fallback. Do not use sslip.io or nip.io as the permanent address because certificate issuance for their shared registered domains can be affected by other users.

Let's Encrypt supports short-lived IP-address certificates as of 2026, but the stock Caddy automatic-public-HTTPS path used by this plan still documents public certificates around qualifying hostnames. Do not add a second ACME client and six-day IP-certificate renewal path to avoid choosing a hostname.

Verify DNS before starting Caddy:

~~~bash
export CLICK_BRIDGE_DOMAIN='ACTUAL_CLICK_BRIDGE_DOMAIN'
dig +short A "$CLICK_BRIDGE_DOMAIN"
dig +short AAAA "$CLICK_BRIDGE_DOMAIN"
~~~

Expected: the A record is exactly the OCI public IPv4. AAAA is empty unless the OCI VM has a configured, firewall-open, tested public IPv6 address. Remove any stale AAAA before Caddy startup. Do not start Caddy until this passes; repeated ACME failures create avoidable backoff.

- [ ] **Step 4: Verify both OCI and host firewalls**

Required public ingress:

- TCP 80 for ACME redirect/challenge;
- TCP 443 for HTTPS/WSS.

Port 8080 must not be public. Record the OCI NSG/security-list rule and host-firewall command in docs/oci-deployment.md.

- [ ] **Step 5: Transfer the exact repository and verify VM prerequisites**
...
ssh "$OCI_SSH_TARGET" 'sudo mkdir -p /opt/click-bridge/releases /opt/click-bridge/shared && sudo chown -R "$USER":"$USER" /opt/click-bridge && command -v rsync && docker version && docker compose version'
...
Replace every ACTUAL value before execution and verify CLICK_BRIDGE_RELEASE matches only fourteen UTC digits plus T and Z. The --delete target is the newly created, exact release directory; never point it at /opt/click-bridge, a home directory, or an unresolved variable. If rsync, Docker Engine, or the Compose plugin check fails, stop deployment and install the missing prerequisite from the operating-system vendor or Docker's official instructions for the operating system recorded in Task 1. Record the exact successful installation commands in docs/oci-deployment.md, then rerun all checks.

On the VM, create /opt/click-bridge/shared/secrets.env once with CLICK_BRIDGE_DOMAIN, PHONE_TOKEN, and MAC_TOKEN and set mode 0600. It is outside every transferred release.

- [ ] **Step 6: Build on the OCI VM**

Build on the VM so Docker selects the VM's recorded amd64 or arm64 architecture:

~~~bash
export CLICK_BRIDGE_RELEASE='ACTUAL_RELEASE_FROM_STEP_5'
...
- [ ] **Step 7: Verify the public application**
...
Then run smoke-relay.mjs against "wss://${CLICK_BRIDGE_DOMAIN}/ws" using the real role tokens supplied only through environment variables.
...
- only Caddy ports exposed publicly.
...
- [ ] **Step 8: Verify rollback and recovery behavior**
...
- [ ] **Step 8: Make v5 canonical before the Milestone 1 stop**
...
| --- | --- | --- | --- |
| Primary relay | Existing OCI us-sanjose-1 VM | Fly.io | Already owned, fixed placement, controllable warm process |
